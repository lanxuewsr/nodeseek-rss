...
def _migrate(conn: sqlite3.Connection) -> None:
    ...
    if notif_cols:
        if "telegram_status" not in notif_cols:
            conn.execute(
                "ALTER TABLE notifications ADD COLUMN telegram_status TEXT NOT NULL DEFAULT 'skipped'"
            )
        if "email_status" not in notif_cols:
            conn.execute(
                "ALTER TABLE notifications ADD COLUMN email_status TEXT NOT NULL DEFAULT 'skipped'"
            )
        if "error" not in notif_cols:
            conn.execute("ALTER TABLE notifications ADD COLUMN error TEXT")
    ...
    with _conn() as conn:
        conn.executescript("""
            ...
            CREATE TABLE IF NOT EXISTS notifications (
                ...
                status          TEXT NOT NULL DEFAULT 'sent',  -- overall: 'sent' | 'failed'
        ...
        conn.execute(
            """INSERT INTO notifications
               (post_id, keyword, title, link, category, author, status,
                telegram_status, email_status, error, sent_at)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (
                post_id,
                keyword,
    ...
    with _conn() as conn:
        rows = conn.execute(
            """SELECT post_id,
                      ...
                      GROUP_CONCAT(DISTINCT telegram_status) AS telegram_statuses,
                      GROUP_CONCAT(DISTINCT email_status) AS email_statuses,
...
def cleanup_database() -> None:
    cleanup_old_seen(keep_days=config.SEEN_POSTS_RETENTION_DAYS)
    cleanup_old_notifications(keep_days=config.NOTIFICATIONS_RETENTION_DAYS)
